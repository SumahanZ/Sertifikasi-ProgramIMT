...
@section("content")
            ...
            <div class="card-body">
                ...
                <form action="/order/{{ $order->id }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nama" name="nama" value="{{ $order->customer->nama }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="alamat" class="form-label">Alamat</label>
                        <textarea class="form-control" id="alamat" name="alamat" required>{{ $order->customer->alamat }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label for="no_telp" class="form-label">No. Telepon</label>
                        <input type="tel" class="form-control" id="no_telp" name="no_telp" value="{{ $order->customer->no_telp }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="id_card" class="form-label">ID Card</label>
                        <input type="text" class="form-control" id="id_card" name="id_card" value="{{ $order->customer->id_card }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <output for="jumlah_kendaraan" id="jumlah_kendaraan_label">{{ count($vehiclesFromOrder) }}</output>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('jumlah_kendaraan').value = {{ count($vehiclesFromOrder) }};
        ...
        function updateVehicleSelection() {
            ...
            for (var i = 0; i < numberOfVehicles; i++) {
                ...
                vehicleDiv.innerHTML = '<label class= "mt-3" for="vehicle_' + i + '">Select Vehicle ' + (i + 1) + ':</label>' +
